...
use speed_daemon::wire::{ReadFrom, WriteTo};
...
async fn test_session() {
...

    {
        ...
        speed_daemon::wire::IAmDispatcher { roads: vec![123] }
            .write_to(&mut write)
            .await
            .unwrap();
        let ticket = timeout(
            Duration::from_millis(1000),
            speed_daemon::wire::Ticket::read_from(&mut read),
        )
        .await
        .unwrap()
        .unwrap();
...
async fn test_invalid_message() {
    ...
    assert_eq!(
        speed_daemon::wire::Error {
            msg: "invalid message: 0x20".to_string()
        },
        speed_daemon::wire::Error::read_from(&mut read)
            .await
            .unwrap()
    );
...
async fn test_heartbeat_camera() {
    ...
    speed_daemon::wire::WantHeartbeat { interval: 1 }
        .write_to(&mut write)
        .await
        .unwrap();

    for i in 0..10 {
        if let Ok(Ok(speed_daemon::wire::Heartbeat)) = timeout(
            Duration::from_millis(1000),
            speed_daemon::wire::Heartbeat::read_from(&mut read),
        )
        .await
        {
            // ok
...
async fn spawn_app() -> (String, u16) {
    ...
    tokio::spawn(async move {
        speed_daemon::run(listener).await.expect("run failed");
    });
